...
class CGAN():
	def __init__(self, generator1,generator2, discriminator, data):
		...
		self.G_solver = tf.train.AdamOptimizer().minimize(self.G_loss, var_list=self.generator2.vars)#+self.generator2.vars)#G1,G2的vars
	...
	def train(self, sample_dir, ckpt_dir='ckpt', training_epoches = 1000000, batch_size = 64):
		...
		for epoch in range(training_epoches):
			...
			if epoch % 200 == 0:
				...
				x_sorted=self.sess.run(
					self.X_sorted,
					feed_dict={self.X: X_b,self.y_stack:y_stack_b, self.z1: sample_z(batch_size, self.z_dim),self.z2: sample_z(batch_size, self.z_dim)})

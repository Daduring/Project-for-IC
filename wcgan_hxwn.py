...
class CGAN():
	def __init__(self, generator1,generator2, discriminator, data):
		...
		self.D_loss_fake=self.weight*tf.reduce_mean(self.D_fake) 
		self.D_loss=self.D_loss_real+self.D_loss_fake
		self.G_loss = -tf.reduce_mean(self.D_fake)

		# solver
		self.D_solver = tf.train.RMSPropOptimizer(LR).minimize(self.D_loss, var_list=self.discriminator.vars)
		self.G_solver = tf.train.RMSPropOptimizer(LR).minimize(self.G_loss, var_list=self.generator2.vars+self.generator1.vars)#G1,G2的vars

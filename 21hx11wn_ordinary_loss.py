...
class CGAN():
	def __init__(self, generator1,generator2, data):
		...
		self.G_loss = tf.reduce_mean(tf.reduce_mean(tf.squared_difference(self.G_sample, self.X_sorted), 1))

		# solver
		#self.D_solver = tf.train.AdamOptimizer().minimize(self.D_loss, var_list=self.discriminator.vars)
		self.G_solver = tf.train.AdamOptimizer().minimize(self.G_loss, var_list=self.generator1.vars+self.generator2.vars)#G1,G2的vars
    ...
    #         			feed_dict={self.X: X_b, self.y: y_b, self.y_stack: y_stack_b, self.z1: sample_z(batch_size, self.z_dim),self.z2: sample_z(batch_size, self.z_dim)})
				G_loss_curr = self.sess.run(
						self.G_loss,
						feed_dict={self.X: X_b,self.y: y_b,self.y_stack:y_stack_b, self.z1: sample_z(batch_size, self.z_dim),self.z2: sample_z(batch_size, self.z_dim)})
				print('Iter: {}; G_loss: {:.4}'.format(epoch, G_loss_curr))
...
if __name__ == '__main__':
	...
	data=channel_hx_plus_wn('mlp')

	# run
	cgan = CGAN(generator1,generator2, data)
	# sess=cgan.sess

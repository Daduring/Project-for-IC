...
class CGAN():
	def __init__(self, generator, discriminator, data):
		...
		self.X = tf.placeholder(tf.float32, shape=[None, self.X_dim])#real channel output
		self.z = tf.placeholder(tf.float32, shape=[None, self.z_dim])#random numbers to generate data
		#self.z2 = tf.placeholder(tf.float32, shape=[self.batch_size, self.z_dim])#random numbers to generate w
		self.y = tf.placeholder(tf.float32, shape=[None, self.y_dim])#channel input repeated y_dim times
		#self.y_stack=tf.placeholder(tf.float32,shape=[self.batch_size,self.X_dim])#channel input repeated z_dim times

		# nets
		self.G_sample = self.generator(concat(self.z,self.y))#generated data
